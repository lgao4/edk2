  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Unable to find VPD Data RAW section\n"));
    return Status;
  }
  ASSERT (FvSection->Type == EFI_SECTION_RAW);
  ASSERT (PcdGet32 (PcdOvmfVpdMemSize) >= SECTION_SIZE (FvSection));
  CopyMem ((VOID*)(UINTN) PcdGet32 (PcdVpdBaseAddress), (VOID*) (FvSection + 1), SECTION_SIZE (FvSection) - sizeof (*FvSection));
